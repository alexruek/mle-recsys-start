...
class EventStore:
    ...
    def put(self, user_id, item_id):
        ...
        """
        user_events = self.events.get(user_id, [])  # Получаем текущие события пользователя
        self.events[user_id] = [item_id] + user_events[: self.max_events_per_user - 1]  # Обновляем список событий

    def get(self, user_id, k):
        """
        Возвращает последние k событий для пользователя.
        """
        user_events = self.events.get(user_id, [])[:k]  # Получаем последние k событий
        return user_events
